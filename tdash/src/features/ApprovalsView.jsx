...
const ApprovalsView = ({ isDark, user }) => {
    ...
    useEffect(() => {
        fetchApprovals();
        fetchEvents();
    }, [statusFilter, eventFilter]); // Refetch when filters change
    ...
    const handleDeleteApproval = async (approvalId) => {
        // detail view now handles api call via modal, this updates local state
    ...
    const handleBackFromDetail = () => {
        setSelectedApproval(null);
        fetchApprovals(); // 1.2 Re-fetch to prevent stale data
    };
    ...
    filteredApprovals.sort((a, b) => {
        switch (sortBy) {
            ...
            case 'priority':
                const pMap = { CRITICAL: 3, URGENT: 2, STANDARD: 1 };
            ...
            <div className={`sticky top-0 z-10 border-b ${isDark ? 'bg-[#0A0A0A]/95 backdrop-blur-xl border-gray-800/60' : 'bg-white/95 backdrop-blur-xl border-gray-200'}`}>
                <div className="px-6 py-4">
                    ...
                    <div className="relative group max-w-xs">
                        <div className={`flex items-center gap-2 px-4 py-3 rounded-xl border text-sm cursor-pointer transition-colors ${isDark
                            ? 'bg-[#1A1A1A] border-gray-800 text-gray-200 hover:border-gray-700'
                            ...
                            <select
                                ...
                                className={`w-full bg-transparent appearance-none focus:outline-none cursor-pointer pr-8 font-medium ${!eventFilter ? 'text-indigo-500' : ''}`}
                            >
                            ...
                            <ChevronDown className={`w-4 h-4 absolute right-4 pointer-events-none ${isDark ? 'text-gray-500' : 'text-gray-400'}`} />
                        </div>
                    </div>
                </div>
            </div>
            ...
            <div className="px-6 pt-5 pb-2">
                <div className="flex flex-col md:flex-row md:items-center gap-3 justify-between">
                    <div className="flex flex-wrap items-center gap-3 flex-1">
                        ...
                        <div className="flex items-center gap-1.5 overflow-x-auto">
                            <button
                                ...
                                className={`px-3.5 py-2 rounded-lg text-xs font-semibold whitespace-nowrap transition-all ${statusFilter === 'ALL'
                                    ? isDark
                            ...
                            {Object.entries(STATUS_CONFIG).map(([key, config]) => {
                                ...
                                // 1.3 Fix: Count based on current list (filtered by event)
                                const count = approvals.filter(a => a?.status === key).length;
                                return (
                                    <button
                                        key={key}
                                        onClick={() => setStatusFilter(statusFilter === key ? 'ALL' : key)}
                                        className={`flex items-center gap-1.5 px-3.5 py-2 rounded-lg text-xs font-semibold whitespace-nowrap transition-all ${statusFilter === key
                    ...
                    <div className="relative group">
                        <div className={`flex items-center gap-2 px-3 py-2.5 rounded-xl border text-sm cursor-pointer ${isDark
